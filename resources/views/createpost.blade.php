            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <form action="{{route('posts.store')}}" method="POST" id="form" enctype="multipart/form-data">
                        ...
                        <select name="category_id" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" type="text" required autofocus>
                            @foreach ($category as $category)
                            <option value='{{$category->id}}'>{{$category->title}}</option>
                            @endforeach
                        </select>
                        <label for="desc" style="font-weight:bold; color:#333">Description</label>
                        <textarea style=" width: 100%; height: 100px; padding: 16px 20px; border: none; border-radius: 4px; background-color: #f1f1f1;" name="desc" type="text" placeholder="Your user name"></textarea>
